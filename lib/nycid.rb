...
module NYCID
  ...
  def validate_email(email_address:)
    ...
    url = "#{ENV['NYC_ID_WEB_SERVICES_URL']}#{EMAIL_VALIDATION_ENDPOINT}?emailAddress=#{email_address}&target=#{Base64::encode64(Devise.omniauth_configs[:saml].strategy[:assertion_consumer_service_url])}"
    return url
  end
